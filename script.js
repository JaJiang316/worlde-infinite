...
document.addEventListener('keyup', () => click(event.key));
...
const delLetter = () => {
    if(currentTile > 0 && gameOver == false){
        currentTile--;
        const board = document.querySelector('.board');
        const row = board.children[currentRow];
        const tile = row.children[currentTile];
...
const checkRow = () => {
    ...
    const guessWord = [];
    row.forEach(tile => {
        guessWord.push({letter: tile.getAttribute('data'), color: 'grey-overlay'});
    });
